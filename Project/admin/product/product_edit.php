...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["name"]) {
            ...
            if ($_FILES["image"]["size"] > 5000000) {
                ...
                if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg") {
                    ...
                    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                        ...
                        $stmt->bind_param("ssssss", $product_name, $quantity, $price, $category, $convertImage, $product_code);
        ...
        $stmt = $conn->prepare("UPDATE product_list SET Product_name=?, Quantity=?, Price=?, Category=? WHERE Product_code=?");
        $stmt->bind_param("sssss", $product_name, $quantity, $price, $category, $product_code);
